...
class MediaLibraryController extends Controller {
	...
	public function popup_media_gallery() {
		return view ('backend.error500');
		// route di bawah masih terdapat bug
	...
	public function upload(Request $request) {
		...
		if(isset($info[0]->name) && (!isset($info[0]->error)))
		{
			...
			if($data->save()){
				return Redirect::to('/backend/media-library/')->with('success', "Data saved successfully")->with('mode', 'success');
			}
		}
	}
